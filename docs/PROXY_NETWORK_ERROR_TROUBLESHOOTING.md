...
[Proxy] ❌ Error: {
  requestId: '...',
  backendUrl: 'http://52.78.150.124:3000/api/teas',
  method: 'GET',
  path: 'teas',
  errorName: 'TypeError',
...
Vercel 서버리스 함수가 백엔드 서버(`52.78.150.124`)에 **TCP 연결을 할 수 없음**을 의미합니다.
...
| 현재 (잘못된 설정) | 수정 (올바른 설정) |
|-------------------|-------------------|
| `http://52.78.150.124:3000` | `http://52.78.150.124` |

**Vercel 대시보드에서 수정:**
1. Project → **Settings** → **Environment Variables**
2. `BACKEND_URL` 값을 `http://52.78.150.124`로 변경 (포트 번호 제거)
3. **Redeploy** 실행
...
curl -v http://52.78.150.124/api/teas

# 포트 3000 (직접) - 방화벽에서 3000 허용한 경우
curl -v http://52.78.150.124:3000/api/teas
```

- 포트 80만 응답하면 → `BACKEND_URL`을 `http://52.78.150.124`로 설정
- 포트 3000도 응답하면 → `BACKEND_URL`을 `http://52.78.150.124:3000`로 설정 가능 (방화벽 확인)
...
- [ ] Nginx 사용 시: `BACKEND_URL=http://52.78.150.124` (포트 80)
