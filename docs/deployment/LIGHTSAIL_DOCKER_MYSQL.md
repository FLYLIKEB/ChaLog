...
RDS 대신 Lightsail 인스턴스 내부에 Docker를 사용하여 MySQL을 설치하는 가이드입니다.
...
### 5. 데이터 마이그레이션 (RDS → Docker MySQL)

#### 옵션 1: mysqldump 사용 (권장)

```bash
# RDS에서 덤프 생성 (로컬에서 실행)
mysqldump -h database-1.cnyqy8snc0sl.ap-northeast-2.rds.amazonaws.com \
  -u admin -p \
  --single-transaction \
  --routines \
  --triggers \
