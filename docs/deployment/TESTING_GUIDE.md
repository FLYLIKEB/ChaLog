...
- 직접 접근: `http://YOUR_LIGHTSAIL_IP:3000/health`
- Nginx를 통한 접근: `http://YOUR_LIGHTSAIL_IP/health`
...
```bash
# 로그인 (예시)
curl -X POST http://YOUR_LIGHTSAIL_IP/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username": "test", "password": "test"}'

# JWT 토큰으로 보호된 엔드포인트
curl http://YOUR_LIGHTSAIL_IP/api/users/me \
...
```bash
# 데이터 생성 (예시)
curl -X POST http://YOUR_LIGHTSAIL_IP/api/notes \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN" \
  -d '{"title": "Test Note", "content": "Test Content"}'

# 데이터 조회
curl http://YOUR_LIGHTSAIL_IP/api/notes \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
```bash
time curl -s http://YOUR_LIGHTSAIL_IP/health > /dev/null
```
...
ab -n 100 -c 10 http://YOUR_LIGHTSAIL_IP/health
```
...
# 여러 요청 동시 실행
for i in {1..10}; do
  curl -s http://YOUR_LIGHTSAIL_IP/health &
done
wait
```
