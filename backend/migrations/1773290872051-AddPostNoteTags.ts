...
export class AddPostNoteTags1773290872051 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`tea_wishlists\` DROP FOREIGN KEY \`tea_wishlists_ibfk_1\``);
        await queryRunner.query(`ALTER TABLE \`tea_wishlists\` DROP FOREIGN KEY \`tea_wishlists_ibfk_2\``);
        await queryRunner.query(`DROP INDEX \`IDX_password_resets_tokenHash\` ON \`password_resets\``);
        await queryRunner.query(`DROP INDEX \`IDX_tea_wishlists_teaId\` ON \`tea_wishlists\``);
        await queryRunner.query(`DROP INDEX \`IDX_tea_wishlists_userId\` ON \`tea_wishlists\``);
        await queryRunner.query(`DROP INDEX \`unique_tea_user_wishlist\` ON \`tea_wishlists\``);
        await queryRunner.query(`DROP INDEX \`IDX_post_images_postId\` ON \`post_images\``);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE INDEX \`IDX_post_images_postId\` ON \`post_images\` (\`postId\`)`);
        await queryRunner.query(`CREATE UNIQUE INDEX \`unique_tea_user_wishlist\` ON \`tea_wishlists\` (\`teaId\`, \`userId\`)`);
        await queryRunner.query(`CREATE INDEX \`IDX_tea_wishlists_userId\` ON \`tea_wishlists\` (\`userId\`)`);
        await queryRunner.query(`CREATE INDEX \`IDX_tea_wishlists_teaId\` ON \`tea_wishlists\` (\`teaId\`)`);
        await queryRunner.query(`CREATE UNIQUE INDEX \`IDX_password_resets_tokenHash\` ON \`password_resets\` (\`tokenHash\`)`);
        await queryRunner.query(`ALTER TABLE \`tea_wishlists\` ADD CONSTRAINT \`tea_wishlists_ibfk_2\` FOREIGN KEY (\`userId\`) REFERENCES \`users\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`tea_wishlists\` ADD CONSTRAINT \`tea_wishlists_ibfk_1\` FOREIGN KEY (\`teaId\`) REFERENCES \`teas\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
