...
# 5. RDS 보안 그룹 안내
echo -e "${YELLOW}📋 5단계: RDS 보안 그룹 설정${NC}"
echo -e "${BLUE}🔐 RDS 보안 그룹에 Lightsail IP 추가 필요${NC}"
echo ""
echo "다음 단계를 수행하세요:"
echo "1. AWS 콘솔 → RDS → 데이터베이스 선택"
echo "2. '연결 및 보안' 탭 → VPC 보안 그룹 클릭"
echo "3. 인바운드 규칙 편집 → 규칙 추가"
echo "   - 유형: MySQL/Aurora (3306)"
echo "   - 소스: $LIGHTSAIL_IP/32"
echo "4. 규칙 저장"
echo ""

# 6. 배포 안내
echo -e "${YELLOW}📋 6단계: 애플리케이션 배포${NC}"
echo ""
...
echo "[ ] RDS 보안 그룹에 Lightsail IP 추가"
echo "[ ] 환경 변수 업로드 완료"
echo "[ ] 애플리케이션 배포 완료"
echo "[ ] Nginx 설정 완료"
