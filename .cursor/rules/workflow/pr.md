...
- 각 리뷰마다 AI 도구를 사용하여 "이 리뷰 반영해줘"라고 요청하면 코드를 자동으로 수정할 수 있습니다
- 파일 변경이 감지되면 자동으로 커밋, 푸시, 리뷰 스레드에 답글 작성, resolve가 수행됩니다
- 리뷰 스레드에 직접 답글이 작성되며, 일반 PR 코멘트가 아닌 리뷰 스레드의 첫 번째 코멘트에 답글이 달립니다
